...
async function candidates() {
  ...
  for (const s of subs) {
    ...
    out.push({ sub: s, shop, owner });
  }
  return out;
}

function summarise(c: any) {
  const tier = resolveSub(c.sub);
  const { amountCents, currency, interval } = stripeAmount(c.sub);
...
async function main() {
  ...
  if (flag("--list") || argv.length === 0) {
    console.log(`${list.length} Stripe-billed active subscription(s):\n`);
  ...
  if (flag("--send")) {
    ...
    for (const c of list) {
      ...
      if (ok && !dry) await Subscription.updateOne({ _id: c.sub._id }, { $set: { migrationEmailedAt: new Date() } });
      n++;
    }
